...
export class ClassroomYearAssignmentRepository {
  ...
  insertMany(entities: ClassroomYearAssignmentEntity[]) {
    return this.database.insertInto('classroomYearAssignment').values(entities).execute();
  }

  async countByClassroomYearIdsAndTeacherSubjectIds(classroomYearIds: number[], teacherSubjectIds: number[]) {
