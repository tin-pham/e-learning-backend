  ...
  find(dto: CategoryGetListDTO) {
    ...
    if (withCourseCount) {
      query = this.database
        ...
        .select(({ fn }) => ['category.id', 'category.name', 'category.createdBy', fn.count('course.id').as('courseCount')])
        .where('category.deletedAt', 'is', null)
        .groupBy(['category.id', 'category.name']);
    } else {
      query = this.database.selectFrom('category').select(['id', 'name', 'createdBy']).where('deletedAt', 'is', null);
    }
