...
const { NAME: TEACHER_SUBJECT_NAME, SCHEMA: TEACHER_SUBJECT_SCHEMA } =
  DATABASE_TABLE.TEACHER_SUBJECT;
...
export async function up(database: DatabaseService): Promise<void> {
  await database.schema
    ...
    .addColumn(SCHEMA.TEACHER_SUBJECT_ID, 'varchar(50)', (column) =>
      column.references(`${TEACHER_SUBJECT_NAME}.${TEACHER_SUBJECT_SCHEMA.ID}`),
    )
