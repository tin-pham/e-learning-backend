...
const { NAME: CLASSROOM_NAME, SCHEMA: CLASSROOM_SCHEMA } = DATABASE_TABLE.CLASSROOM;
...
export async function up(database: DatabaseService): Promise<void> {
  await database.schema
    ...
    .addColumn(SCHEMA.CLASSROOM_ID, 'integer', (column) => column.references(`${CLASSROOM_NAME}.${CLASSROOM_SCHEMA.ID}`))
    .addColumn(SCHEMA.YEAR_ID, 'integer', (column) => column.references(`${YEAR_NAME}.${YEAR_SCHEMA.ID}`))
    .addColumn(SCHEMA.FORM_TEACHER_ID, 'varchar(50)', (column) => column.references(`${TEACHER_NAME}.${TEACHER_SCHEMA.ID}`))
