...
const { NAME, SCHEMA } = DATABASE_TABLE.ROLE_MENU;
const { NAME: MENU_NAME, SCHEMA: MENU_SCHEMA } = DATABASE_TABLE.MENU;
const { NAME: ROLE_NAME, SCHEMA: ROLE_SCHEMA } = DATABASE_TABLE.ROLE;
...
export async function up(database: DatabaseService): Promise<void> {
  await database.schema
    ...
    .addColumn(SCHEMA.MENU_ID, 'integer', (column) => column.references(`${MENU_NAME}.${MENU_SCHEMA.ID}`).notNull())
    .addColumn(SCHEMA.ROLE_ID, 'integer', (column) => column.references(`${ROLE_NAME}.${ROLE_SCHEMA.ID}`).notNull())
