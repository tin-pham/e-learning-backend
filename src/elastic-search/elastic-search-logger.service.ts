...
import { ElasticSearchLoggerOptions } from './elastic-search-logger.options';
import { ElasticSearchLoggerUtil } from './elastic-search-logger.util';
...
export class ElasticSearchLoggerService {
  ...
  public async log<T>(
    ...
    if (!!!indice) {
      indice = ElasticSearchLoggerUtil.getRollingIndex(
  ...
  public async getDetail() {
    const result = await this.client.search({
      index: ElasticSearchLoggerUtil.getRollingIndex(
        this.options.indexPrefix,
        this.options.rollingOffsetMode,
      ),
    });
